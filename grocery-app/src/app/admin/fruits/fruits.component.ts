...
import { ApiService } from 'src/app/shared/fruits.service'; 
@Component({
  selector: 'app-fruits',
...
export class FruitsComponent implements OnInit {
  ...
  userModelObj : FruitModel = new FruitModel();
  userData !: any;
  showAdd !: boolean;
  showUpdate !: boolean;
  submitted =false;
constructor(private formbuilder: FormBuilder,private fruits : ApiService) { }
ngOnInit(): void {
...
postUserDetails(){
  ...
  this.fruits.postUser(this.userModelObj).subscribe(res=>{
...
getUserDetails(){
    this.fruits.getUser().subscribe(res=>{
      this.userData = res;
    })
}
deletUserDetails(row : any){
  this.fruits.deleteUser(row.id).subscribe(res=>{
    alert("User Deleted");
    this.getUserDetails();
...
updateUserDetails(){
  ...
  this.fruits.updateUser(this.userModelObj,this.userModelObj.id)
  .subscribe(res=>{
    console.log(res);
